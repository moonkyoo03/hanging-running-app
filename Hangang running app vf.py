...
# 기본 설정
# ──────────────────────────────────────────────────────────
st.set_page_config(page_title="한강 러닝 루트 추천 v3", layout="wide")
...
st.markdown(
    ...
    .cal-cell.selected::before { background: #0f766e; }
    .cal-cell.today {
        border-color: #3b82f6;
        background: #eff6ff;
    }
    .cal-cell.today::before { background: #3b82f6; }
    .cal-cell.has-run {
        border-color: #10b981;
        background: linear-gradient(160deg, #f0fdf4 0%, #dcfce7 100%);
    }
    .cal-cell.has-run::before { background: #10b981; }
    .cal-cell-day {
        font-size: 1.05rem;
        font-weight: 800;
        color: #1e293b;
    ...
    .cal-cell-day.sun { color: #ef4444; }
    .cal-cell-day.sat { color: #3b82f6; }
    .cal-cell-tag {
        font-size: 0.58rem;
        font-weight: 700;
        color: #ffffff;
    ...
    .cal-cell-tag.run-tag { background: #10b981; }
    .cal-cell-km {
        font-size: 0.68rem;
        color: #059669;
        font-weight: 800;
    ...
    div[data-testid="element-container"]:has(button[kind="secondary"]) button {
        height: 86px !important;
        width: 100% !important;
...
def validate_password(pw: str):
    """비밀번호 유효성 검사. (문제 없으면 None, 문제 있으면 오류 메시지 반환)"""
    import re
    if len(pw) < 8:
    ...
    if not re.search(r"[!@#$%^&*()_\-+=\[\]{};':\"\\|,.<>/?`~]", pw):
        return "비밀번호에 특수문자(!@#$ 등)가 포함되어야 합니다."
    return None
...
def register_user(username, password):
    ...
    if p_err:
        return False, p_err
    users[username] = {"pw": hash_pw(password), "friends": [], "created_at": datetime.now().strftime("%Y-%m-%d %H:%M:%S")}
    save_users(users)
    return True, "회원가입 완료!"
...
def generate_random_route(target_km, route_mode, tolerance_ratio=0.08, max_try=30, start_spot=None):
    ...
    for attempt in range(max_try):
        ...
        # 사용자가 출발지를 지정한 경우 그 지점을 시작으로, 아니면 랜덤
...
# ── 한강공원 기본 편의시설 데이터 (API 폴백용) ─────────────
_HANGANG_TOILETS_BASE = [
    ("여의도한강공원 화장실①", 37.5272, 126.9316),
    ("여의도한강공원 화장실②", 37.5258, 126.9291),
    ("여의도한강공원 화장실③", 37.5281, 126.9350),
    ("반포한강공원 화장실①",   37.5083, 126.9968),
    ("반포한강공원 화장실②",   37.5097, 126.9988),
    ("뚝섬한강공원 화장실①",  37.5296, 127.0713),
    ("뚝섬한강공원 화장실②",  37.5308, 127.0742),
    ("잠실한강공원 화장실",    37.5186, 127.0872),
    ("망원한강공원 화장실①",  37.5553, 126.8979),
    ("망원한강공원 화장실②",  37.5568, 126.8996),
    ("이촌한강공원 화장실①",  37.5222, 126.9712),
    ("이촌한강공원 화장실②",  37.5238, 126.9738),
    ("난지한강공원 화장실",   37.5661, 126.8763),
    ("선유도공원 화장실",     37.5380, 126.8990),
    ("노들섬 화장실",         37.5169, 126.9600),
    ("세빛섬 화장실",         37.5072, 126.9978),
    ("광나루 화장실",         37.5445, 127.1090),
    ("암사생태공원 화장실",   37.5538, 127.1335),
]

_HANGANG_CONV_BASE = [
    ("GS25 여의도한강점",   37.5270, 126.9329),
    ("CU 여의도한강점",     37.5264, 126.9309),
    ("세븐일레븐 여의도점", 37.5278, 126.9355),
    ("CU 반포한강점",       37.5089, 126.9974),
    ("GS25 반포한강점",     37.5094, 126.9983),
    ("CU 뚝섬한강점",       37.5299, 127.0720),
    ("GS25 잠실한강점",     37.5191, 127.0879),
    ("CU 망원한강점",       37.5558, 126.8983),
    ("GS25 이촌한강점",     37.5229, 126.9722),
    ("CU 난지한강점",       37.5664, 126.8772),
    ("GS25 뚝섬유원지점",  37.5305, 127.0668),
    ("세븐일레븐 잠실나루점", 37.5180, 127.0833),
    ("CU 선유도점",          37.5384, 126.8987),
    ("GS25 노들섬점",        37.5170, 126.9601),
]


def _haversine_filter(items, route_coords, radius_m):
    """route_coords(lon,lat) 중 어느 한 점과 radius_m 이내인 시설만 반환."""
    result = []
    for name, lat, lon in items:
        for rlon, rlat in route_coords:
            if haversine_km(lat, lon, rlat, rlon) * 1000 <= radius_m:
                result.append((lat, lon, name))
                break
    return result


def fetch_amenities(route_coords, radius_m=400):
    """루트 좌표(lon,lat) 주변 한강공원 편의시설 반환."""
    if not route_coords:
        return {"toilets": [], "convenience": []}
    toilets = _haversine_filter(_HANGANG_TOILETS_BASE, route_coords, radius_m)
    conv    = _haversine_filter(_HANGANG_CONV_BASE,    route_coords, radius_m)
    return {"toilets": toilets, "convenience": conv}


def build_map(result=None, amenities=None):
    ...
    # ── 편의시설 마커 ──
    if amenities:
        toilet_group = folium.FeatureGroup(name="🚻 화장실", show=True)
        conv_group   = folium.FeatureGroup(name="🏪 편의점", show=True)

        for lat, lon, name in amenities.get("toilets", []):
            folium.Marker(
                location=(lat, lon),
                tooltip=f"🚻 {name}",
                popup=folium.Popup(f"<b>🚻 화장실</b><br>{name}", max_width=160),
                icon=folium.DivIcon(
                    html=f"""<div style="
        ...
        for lat, lon, name in amenities.get("convenience", []):
            folium.Marker(
                location=(lat, lon),
                tooltip=f"🏪 {name}",
                popup=folium.Popup(f"<b>🏪 편의점</b><br>{name}", max_width=160),
                icon=folium.DivIcon(
                    html=f"""<div style="
...
if not st.session_state["user"]:
    ...
    with col_c:
        ...
        if not st.session_state["show_register"]:
            ...
            # 회원가입
            st.markdown("#### 회원가입")
            reg_id = st.text_input("아이디 (3~20자, 영문·숫자·밑줄)", key="reg_id", placeholder="사용할 아이디")
            reg_pw = st.text_input(
                "비밀번호",
                type="password", key="reg_pw",
                placeholder="영문+숫자+특수문자 포함 8자 이상",
                help="8자 이상 · 영문 · 숫자 · 특수문자(!@#$ 등)를 모두 포함해야 합니다.",
            )
            # 실시간 비밀번호 강도 표시
            if reg_pw:
                import re as _re
                checks = {
                    "8자 이상": len(reg_pw) >= 8,
                    "영문 포함": bool(_re.search(r"[A-Za-z]", reg_pw)),
                    "숫자 포함": bool(_re.search(r"\d", reg_pw)),
                    "특수문자 포함": bool(_re.search(r"[!@#$%^&*()_\-+=\[\]{};':\"\\|,.<>/?`~]", reg_pw)),
                }
                passed = sum(checks.values())
                bar_color = ["#ef4444", "#f97316", "#eab308", "#22c55e"][min(passed - 1, 3)] if passed > 0 else "#e2e8f0"
                bar_pct = passed * 25
                check_rows = " &nbsp; ".join(
                    f"{'✅' if ok else '❌'} {label}" for label, ok in checks.items()
                )
                st.markdown(
                    f"""<div style='margin:-0.3rem 0 0.5rem;'>
                    <div style='height:6px;border-radius:3px;background:#e2e8f0;overflow:hidden;'>
                        <div style='width:{bar_pct}%;height:100%;background:{bar_color};transition:width 0.3s;'></div>
                    </div>
                    <div style='font-size:0.78rem;color:#475569;margin-top:4px;'>{check_rows}</div>
                    </div>""",
                    unsafe_allow_html=True,
                )
...
with tab_route:
    ...
    with left:
        ...
        # ── 출발지 선택 ──
        ALL_SPOT_NAMES = ["랜덤 (자동 선택)"] + [s[0] for s in HANGANG_SPOTS]
        selected_start_name = st.selectbox(
            "출발지",
            ALL_SPOT_NAMES,
            index=0,
            help="특정 한강공원이나 랜드마크를 출발점으로 고정할 수 있습니다.",
        )
        start_spot = None
        if selected_start_name != "랜덤 (자동 선택)":
            start_spot = next((s for s in HANGANG_SPOTS if s[0] == selected_start_name), None)
            # 선택된 공원에 시설 정보가 있으면 짧게 표시
            if selected_start_name in PARK_FACILITIES:
                fac = PARK_FACILITIES[selected_start_name]
                diff_icon = DIFFICULTY_COLOR.get(fac.get("difficulty", ""), "")
                st.caption(
                    f"{diff_icon} 난이도: {fac.get('difficulty','?')} · {fac.get('surface','')}  \n"
                    f"💡 {fac.get('tip','')}"
                )
        ...
        if st.button("🎲 루트 생성", type="primary"):
            with st.spinner("다양한 루트 조합 중..."):
                ...
                st.session_state["saved_last"] = False
                st.session_state["amenities"] = None  # 새 루트 생성 시 초기화
    ...
    st.session_state.setdefault("amenities", None)
    st.divider()

    # ── 편의시설 토글 ──
    if result:
        ameni_cols = st.columns([1, 1, 3])
        with ameni_cols[0]:
            show_toilet   = st.toggle("🚻 화장실", value=True,  key="tog_toilet")
        with ameni_cols[1]:
            show_conv     = st.toggle("🏪 편의점", value=True,  key="tog_conv")
        with ameni_cols[2]:
            if st.button("📍 주변 편의시설 검색", key="search_amenity"):
                with st.spinner("루트 주변 편의시설 검색 중..."):
                    raw = fetch_amenities(result["line"], radius_m=400)
                    # 토글 상태 반영
                    st.session_state["amenities"] = {
                        "toilets":     raw["toilets"]     if show_toilet else [],
                        "convenience": raw["convenience"] if show_conv   else [],
                        "_raw": raw,
                    }
        # 토글만 바꾼 경우 raw 재적용
        if st.session_state["amenities"] and "_raw" in st.session_state["amenities"]:
            raw = st.session_state["amenities"]["_raw"]
            st.session_state["amenities"]["toilets"]     = raw["toilets"]     if show_toilet else []
            st.session_state["amenities"]["convenience"] = raw["convenience"] if show_conv   else []
        # 검색 결과 요약
        am = st.session_state["amenities"]
        if am:
            n_t = len(am.get("toilets", []))
            n_c = len(am.get("convenience", []))
            st.caption(f"🚻 화장실 {n_t}개  🏪 편의점 {n_c}개 발견 (루트 반경 400m)")

    map_col, info_col = st.columns([1.8, 1])
    with map_col:
        route_map = build_map(result, amenities=st.session_state.get("amenities"))
    ...
    for week in weeks:
        ...
        for i, d in enumerate(week):
            with cols[i]:
                ...
                day_color_cls = "sun" if is_sun else ("sat" if is_sat else "")
                day_style = ""

                # 태그 배지
                if is_today and not day_runs:
                    tag_html = "<div class='cal-cell-tag'>오늘</div>"
                elif is_today and day_runs:
                    tag_html = "<div class='cal-cell-tag run-tag'>오늘 🏃</div>"
                elif day_runs:
                    tag_html = f"<div class='cal-cell-tag run-tag'>🏃 {len(day_runs)}회</div>"
                else:
                    tag_html = ""
                ...
                st.markdown(
                    ...
                    f"<div class='cal-cell-day {day_color_cls}'>{d.day}</div>"
                    f"{tag_html}"
                    f"{km_html}"
                    f"{pace_html}"
                    f"</div>",
                ...
                # 날짜 선택: 숨겨진 버튼 없이 st.button(label="") 으로 처리
                # 빈 텍스트 버튼 + CSS로 셀 영역 전체를 클릭 가능하게
                if st.button(
                    " ",
                    key=f"pick_{day_key}",
                    use_container_width=True,
                    help=f"{len(day_runs)}회 · {km_d:.1f} km" if day_runs else "기록 없음",
                ):
