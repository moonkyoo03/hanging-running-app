...
def fetch_amenities(route_coords, radius_m=300):
    """
    루트 좌표 리스트(lon, lat) 주변의 편의시설을 Overpass API로 조회.
    반환: {"toilets": [(lat,lon,name), ...], "convenience": [(lat,lon,name), ...]}
    """
    if not route_coords:
    ...
    (
      ...
      node["amenity"="convenience"]({south},{west},{north},{east});
    ...
    for el in elements:
        ...
        if amenity == "toilets":
            toilets.append((lat, lon, name or "화장실"))
        elif amenity in ("convenience",):
            brand = tags.get("brand", "") or tags.get("operator", "")
            display = name or brand or "편의점"
            convenience.append((lat, lon, display))

    return {"toilets": toilets, "convenience": convenience}
